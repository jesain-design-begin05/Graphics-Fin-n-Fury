/* ================================================================
   ...
   Opening Menu — canvas-drawn, no ES modules, no imports.
   On PLAY → hides menu canvas, shows game canvas, boots Game.
================================================================= */
...
class MenuSystem {
    ...
    _init() {
        ...
        window.addEventListener('resize', () => {
            this._resize();
            if (this._gameInstance) this._gameInstance.resize(this.W, this.H, this.dpr);
        });
    ...
    _mkFish() {
        ...
        return { x: dir===1?-80:W+80, y:H*0.12+Math.random()*H*0.7,
                 sz:16+Math.random()*38, vx:(0.35+Math.random()*0.65)*dir,
                 a:0.05+Math.random()*0.09, dir };
    }
    ...
    _loop(ts) {
        ...
        this.lastTime = ts;
        this.elapsed += dt;
        if (!this.isStarted) { this._update(dt); this._draw(); }
    ...
    _update(dt) {
        ...
        for (const btn of this.buttons) {
            if (btn.ripple) {
                btn.ripple.r+=dt*170; btn.ripple.a-=dt*2.0;
    ...
    _drawBG(ctx,W,H,e) {
        ...
        for (let i=0;i<15;i++) {
            ...
            const a2=0.055+Math.sin(e*0.75+i)*0.025;
            ctx.fillStyle=`rgba(110,215,255,${a2})`;
    ...
    _drawBubbles(ctx,e) {
        for (const b of this.bubbles) {
            const r=b.r*(1+Math.sin(e*1.9+b.ph)*0.07);
    ...
    _drawFish(ctx) {
        for (const f of this.fishSilh) {
            ...
            ctx.beginPath(); ctx.moveTo(f.sz,0); ctx.lineTo(f.sz+f.sz*.5,-f.sz*.33);
            ctx.lineTo(f.sz+f.sz*.5,f.sz*.33); ctx.closePath(); ctx.fill();
            ctx.restore();
        }
    }
    ...
    _drawPanel(ctx,W,H,e) {
        const maxW=Math.min(W*0.82,900), maxH=Math.min(H*0.82,680), aspect=900/640;
        let pW,pH;
        ...
        // Shadow
        const ss=1-Math.abs(Math.sin(e*0.9))*0.22;
        const sa=0.22-Math.abs(Math.sin(e*0.9))*0.1;
        const sdw=ctx.createRadialGradient(W/2,pY+pH+22,0,W/2,pY+pH+22,pW*0.44*ss);
        sdw.addColorStop(0,`rgba(0,12,35,${sa})`); sdw.addColorStop(1,'rgba(0,12,35,0)');
        ctx.fillStyle=sdw; ctx.beginPath();
        ctx.ellipse(W/2,pY+pH+22,pW*0.44*ss,14*ss,0,0,Math.PI*2); ctx.fill();
        ...
        this._rr(ctx,pX,pY,pW,pH,20); ctx.stroke(); ctx.restore();
        ...
        ctx.fillStyle=pg; this._rr(ctx,pX,pY,pW,pH,20); ctx.fill();

        // Border
        ctx.strokeStyle=C.panelBorder; ctx.lineWidth=1.8;
        this._rr(ctx,pX,pY,pW,pH,20); ctx.stroke();
        ...
        ctx.fillStyle=sh; this._rr(ctx,pX+1,pY+1,pW-2,pH*0.32,20); ctx.fill();

        // Corner glints
        const gl=0.45+Math.sin(e*2.1)*0.2;
        for (const [gx,gy] of [[pX+22,pY+18],[pX+pW-22,pY+18],[pX+22,pY+pH-18],[pX+pW-22,pY+pH-18]]) {
            ctx.save(); ctx.globalAlpha=gl*0.55; ctx.fillStyle='rgba(200,240,255,0.9)';
            ctx.beginPath(); ctx.arc(gx,gy,3.5,0,Math.PI*2); ctx.fill();
            ctx.globalAlpha=gl*0.25; ctx.beginPath(); ctx.arc(gx,gy,6,0,Math.PI*2); ctx.fill();
            ctx.restore();
        }

        // Water shimmer streaks
        for (const [sx,et,amp] of [[pX+pW*0.14,e,0.045],[pX+pW*0.40,e,0.038],[pX+pW*0.67,e,0.032],[pX+pW*0.84,e,0.028]]) {
            const sxF=sx+Math.sin(et*1.1+sx)*pW*amp;
            if (sxF<pX+6||sxF>pX+pW-6) continue;
            const sa2=0.12+Math.abs(Math.sin(et*0.9+sx))*0.08;
            ctx.save(); ctx.globalAlpha=sa2; ctx.strokeStyle='rgba(180,228,255,0.9)'; ctx.lineWidth=1.5;
            ctx.beginPath(); ctx.moveTo(sxF,pY+pH*0.08); ctx.lineTo(sxF+6,pY+pH*0.18); ctx.stroke();
            ctx.restore();
        }
        ...
        // Subtitle
        ctx.save(); ctx.font=`500 ${pW*0.024}px 'Exo 2',sans-serif`;
        ctx.fillStyle=C.subtitle; ctx.textAlign='center'; ctx.textBaseline='middle';
        ctx.letterSpacing='0.22em';
        ctx.fillText('🐟  AN UNDERWATER SURVIVAL GAME  🐟', cx, pY+pH*0.13);
        ctx.restore();

        // Title
        const tY=pY+pH*0.30, tS=Math.min(pW*0.142,115);
        ctx.save(); ctx.textAlign='center'; ctx.textBaseline='middle';
        ctx.shadowColor='rgba(0,0,0,0.75)'; ctx.shadowBlur=12;
        ctx.shadowOffsetX=3; ctx.shadowOffsetY=4;
        const tg=ctx.createLinearGradient(cx,tY-tS*0.5,cx,tY+tS*0.5);
        tg.addColorStop(0,'#ffd060'); tg.addColorStop(0.55,'#ffb020'); tg.addColorStop(1,'#ff7800');
        ctx.fillStyle=tg; ctx.font=`${tS}px 'Bangers',cursive`;
        ctx.save(); ctx.transform(1,0,-0.10,1,tS*0.055,0);
        ...
        // Tagline
        ctx.save(); ctx.font=`italic 500 ${pW*0.030}px 'Exo 2',sans-serif`;
        ctx.fillStyle=C.tagline; ctx.textAlign='center'; ctx.textBaseline='middle';
        ctx.fillText('Grow big.  Eat smart.  Beware the poisoned one.', cx, pY+pH*0.46);
        ctx.restore();

        // Divider
        const dY=pY+pH*0.525;
        ctx.save();
        const dg=ctx.createLinearGradient(pX+pW*0.15,dY,pX+pW*0.85,dY);
        dg.addColorStop(0,'rgba(60,190,255,0)'); dg.addColorStop(0.5,'rgba(60,190,255,0.35)'); dg.addColorStop(1,'rgba(60,190,255,0)');
        ctx.strokeStyle=dg; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(pX+pW*0.15,dY); ctx.lineTo(pX+pW*0.85,dY); ctx.stroke();
        ctx.restore();

        // Buttons
        const bW=Math.min(pW*0.46,290), bH=Math.min(pH*0.115,52), bG=Math.min(pH*0.030,14);
        const bX=cx-bW/2, bSY=dY+pH*0.045;
        for (let i=0;i<this.buttons.length;i++) {
            const btn=this.buttons[i], by=bSY+i*(bH+bG);
            btn.rect={x:bX,y:by,w:bW,h:bH,r:bH/2};
            this._drawBtn(ctx,btn);
        }

        // Hint bar
        ctx.save(); ctx.font=`400 ${Math.max(10,pW*0.020)}px 'Exo 2',sans-serif`;
        ctx.fillStyle=C.hint; ctx.textAlign='center'; ctx.textBaseline='middle';
        ctx.fillText('USE ARROW KEYS | WASD TO MOVE  ·  SPACE TO SHOOT PEARL', cx, pY+pH*0.935);
        ctx.restore();
    ...
    _drawBtn(ctx,btn) {
        ...
        ctx.fillStyle=fg; this._rr(ctx,x,y,w,h,r); ctx.fill();
        ...
        ctx.fillStyle=sg; this._rr(ctx,x+1,y+1,w-2,h*0.48,r); ctx.fill();

        const fs=Math.max(13,Math.min(h*0.42,18));
        ctx.shadowColor=C.btnShadow; ctx.shadowBlur=5;
        ctx.fillStyle='#ffffff'; ctx.font=`700 ${fs}px 'Exo 2',sans-serif`;
        ctx.textAlign='center'; ctx.textBaseline='middle';
        ctx.fillText(btn.label,x+w/2,y+h/2);

        if (btn.ripple?.a>0) {
            ctx.globalAlpha=btn.ripple.a; ctx.strokeStyle='rgba(255,255,255,0.8)';
    ...
    _hitBtn(px,py) {
        for (const btn of this.buttons) {
            ...
            if (px>=x&&px<=x+w&&py>=y+this.floatY&&py<=y+this.floatY+h) return btn;
        }
        return null;
    }
    ...
    _startGame() {
        ...
        // Game class is defined in game.js (loaded before this file in HTML)
        this._gameInstance = new Game(this.gameCanvas);
